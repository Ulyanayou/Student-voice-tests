...
# Удаление дисциплины
def test_delete_discipline(test_true_login):
    driver = test_true_login 
    discipline_button = driver.find_element('xpath', '//*[@id="root"]/header/div/div[2]/div[1]/a[2]')
    discipline_button.click()
    ...
    delete_discipline_button = driver.find_element('xpath', '//*[@id="root"]/main/div[2]/div/div/button[2]')
    delete_discipline_button.click()

    try:
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(('xpath', '//*[text()="Дисциплина успешно удалена!"]'))
        )

        toast_message = driver.find_element('xpath', '//*[text()="Дисциплина успешно удалена!"]')
        
        WebDriverWait(driver, 10).until(
            EC.visibility_of(toast_message)
        )
        assert "Дисциплина успешно удалена!" in toast_message.text
    except:
        assert False